...
public class ControllerInterface {
    ...
    @FXML
    private void HandleCustomerLoginClicked() throws IOException {    
        window = (Stage) LoginButton.getScene().getWindow();
        isFullscreen = false;
        loadInterface("User/CustomerLogin");
    }

    @FXML
    private void HandleCustomerSignupClicked() throws IOException {    
        window = (Stage) SignupButton.getScene().getWindow();
        isFullscreen = false;
        loadInterface("User/CustomerSignup");
    }
    ...
    private boolean validLogin() {
        accIsTrue = false; // Initialize to false
        String verifyLogin = "SELECT COUNT(1), admin_username, role FROM admin_account WHERE admin_username = ? AND admin_password = ?";
        try {
            PreparedStatement statement = connectDb.prepareStatement(verifyLogin);
            statement.setString(1, adminUsernameTextField.getText().trim());
