...
public class ConfirmOrderController implements Initializable{
    ...
    private void displayDetailDataFromHistory() {
        ...
        String uploadTableView = "SELECT (drink_type || ' ' || drink_name) AS drink, drink_price as price, SUM(drink_quantity) as qty, sum(total_price) as total_price FROM history WHERE customer_id = " + selectedCustomerID + " and status = 'Ordered' group by (drink_type || ' ' || drink_name);";
